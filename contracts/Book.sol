...
contract Book is usingProvable {
    ...
    // Callback for getting winningTeam from Provable
    function __callback(
        bytes32 _myid,
    ...
    // asserts that it is now before game time
    modifier beforeGameTime(uint gameTime) {
        require (now < gameTime);
        _;
    }

    // asserts that it is now after game time
    modifier afterGameTime(uint gameTime) {
        require (now > gameTime);
        _;
    }

    // Thanks https://ethereum.stackexchange.com/questions/2519/how-to-convert-a-bytes32-to-string
    ...
    // Thanks https://ethereum.stackexchange.com/questions/9142/how-to-convert-a-string-to-bytes32
